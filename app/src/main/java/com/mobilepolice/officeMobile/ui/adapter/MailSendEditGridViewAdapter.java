 ...
 * @param <T>
 */
public class MailSendEditGridViewAdapter<T> extends BaseAdapter {

	private List<T> mList = new ArrayList<T>();
	private Activity mActivity;

	public MailSendEditGridViewAdapter(Activity mActivity) {
		super();
	...
	public void appendToList(EmailAttachment infos) {
		if (infos == null) {
			return;
		}
		mList.add((T) infos);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		EmailAttachment affInfos = (EmailAttachment) mList.get(position);
		holder.textView.setText(affInfos.getFileName());
		holder.textView2.setText(EmailAttachment.convertStorage(affInfos.getFileSize()));
